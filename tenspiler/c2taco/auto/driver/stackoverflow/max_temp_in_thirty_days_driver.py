...
if __name__ == "__main__":
    driver, input_vars, max_temp_in_thirty_days = analyze_double_loops(
        file_path="tenspiler/c2taco/cpp/for_synthesis/stackoverflow/max_temp_in_thirty_days.cc",
        func_name="max_temp_in_thirty_days",
        axioms=axioms,
    )
    data, periods = input_vars["data"], input_vars["periods"]
    driver.add_precondition(data.len() >= 1)
    driver.add_precondition(periods.len() >= data.len()+28)
